...
> Uniform initialization: bounded uniformly between ~ [(-1/√f<sub>in</sub>), (1/√f<sub>out</sub>)]
...
> Uniform: bounded uniformly between ~ [+/- sqrt(6/f<sub>in</sub>+f<sub>out</sub>) ]

> Normal: multiply normal distribution by sqrt(2/f<sub>in</sub>+f<sub>out</sub>)
  - np.random.rand(shape) * np.sqrt(2/f<sub>in</sub>+f<sub>out</sub>)
  - or create normal distribution with μ = 0  and σ<sup>2</sup> = sqrt(2/f<sub>in</sub>+f<sub>out</sub>)
...
- both Xavier and He use similar theory →
  ...
  - derived without explicitly considering the type of the distribution
   ...
   > Uniform: [+/- √(6/f<sub>in</sub>)]
   > Normal: normal distribution * √(2/f<sub>in</sub>)
    - or μ = 0  and σ<sup>2</sup> = sqrt(2/f<sub>in</sub>)
